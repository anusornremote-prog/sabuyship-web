...
export async function POST(
  ...
  try {
    ...
    const amount = Number(body.shipping_cost_cn_th)
    if (!Number.isFinite(amount) || amount < 0 || amount > 10_000_000) {
      return NextResponse.json({ error: "ยอดค่าขนส่งไม่ถูกต้อง" }, { status: 400 })
    }
    ...
    const notificationSent = result.customer_id && amount > 0
    ...
    return NextResponse.json({ error: "ไม่สามารถบันทึกค่าขนส่งรอบ 2 ได้" }, { status: 500 })
  }
}
